...
const AdminLogin = ()=>{
    ...
    const onSubmitSuccess = (token,Adminidentity)=>{
      setstatus("success");
      setemail("");
      setpassword("");
      Cookies.set("AdminToken",token,{expires:1});
      localStorage.setItem("AdminToken",token);
      localStorage.setItem("Adminidentity",Adminidentity);
         nav("/AddProducts",{ replace: true });
    }
    ...
    const submitForm = async(event)=>{
      ...
      try{
        const options = {
          ...
          const response = await fetch("https://ecommercebackend-2-tnje.onrender.com/Admin/AdminLogin",options);
    
          const data = await response.json();
          //console.log(data);
    
          if (response.ok===true){
            onSubmitSuccess(data.token,data.Adminidentity);
          }
          else{
